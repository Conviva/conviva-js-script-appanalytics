...
* Scroll depth tracking enabled by default: scroll milestone collection (`conviva_scroll_milestone` / `conviva_scroll_reset`) is now on out of the box with the same default thresholds (25%, 50%, 75%, 100%). Disable via remote configuration (`scrollsCollection.enabled: false`) if not needed.
* Form lifecycle tracking enabled by default: automatic form interaction collection (form view, field blur, submit, validation) is now on by default via remote configuration. PII denylisting for sensitive field names still applies automatically. Disable via remote configuration (`formcc.en: false`) if not needed.
* Baggage message ID support: the SDK can extract and inject a message ID alongside the existing conversation ID into W3C baggage headers on matched outbound requests (`c3.msgid={messageId}`). Remote configuration now uses separate `baggageConfiguration.convId` and `baggageConfiguration.msgId` rule arrays (replacing the legacy flat `attributeExtraction` key). New optional API: `window.apptracker('setMessageId', messageId)` to set the message ID programmatically.

## 2.2.0 (24/MAY/2026)
* Native Bridge Client ID for hybrid WebView apps (Android + iOS): The SDK can now share a single Conviva clientId across a host native app and its embedded WebView so events from both sides land in one combined Conviva session. In the recommended production flow, the native platform sets the clientId in the `Conviva_sdkConfig` cookie on the WebView's cookie store before page load and the JS SDK simply reads the cookie — no JS code change required. If the cookie cannot be set in time, the SDK auto-detects a native bridge on `window` as a fallback (`__ConvivaNativeWebInterface.getClientId()` on Android WebView; `webkit.messageHandlers.__ConvivaiOSGetClientIdInterface.postMessage(null)` returning `Promise<string>` on iOS WKWebView, iOS 14+). When the bridge-supplied clientId differs from the cached value, the SDK fires a `ConvivaClientIdChanged` CustomEvent on `window` so in-tab consumers (e.g. session replay) can refresh their state.
* **Required Cohort Replay version:** When upgrading to DPI SDK v2.2.0 or later, also upgrade Cohort Replay to **v1.0.4 or later** ([release notes](https://github.com/Conviva/conviva-js-replay/releases/tag/v1.0.4)) so replay stays in the same Conviva session as DPI. Older replay versions are not compatible with DPI v2.2.0+ for session synchronization.
...
* Improves pre-initialization queue reliability: errors in queued calls no longer abort remaining calls.
...
* Introduces `trackRevenueEvent()` API to track purchase and revenue events. Events are forwarded as `conviva_revenue_event` and can be used for Business/Revenue Metrics in Pulse. Read more: [Track Revenue Event](https://github.com/Conviva/conviva-js-script-appanalytics?tab=readme-ov-file#more-features)
...
## 1.5.0 (15/DEC/2025)
* Supports manual network request tracking via trackNetworkRequest API. Read more: [Track Network Request Event](https://github.com/Conviva/conviva-js-script-appanalytics?tab=readme-ov-file#more-features)
...
## 0.6.16 (08/NOV/2024)
* Introduces “deviceMetadata” key inside ConvivaAppTracker Initialisation config to set additional device related information. Please refer to [Device Metadata feature](https://github.com/Conviva/conviva-js-script-appanalytics?tab=readme-ov-file#eco-sensor-api-for-setting-device-metadata-from-application-for-sdks) for more details.
...
## 0.6.8 (20/JUL/2024)
* Fixes the issue of missing prototype for XMLHttpRequest thereby causing implementations that use prototype to be unusable.
